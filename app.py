...
with st.sidebar:
    ...
    c_desc = "Akademik iş birliği ve kurumsal analiz teklifleri için ulaşın." if st.session_state.lang == "Türkçe" else "Reach out for academic collaboration."
...
def process_pdf(file_bytes):
    if len(file_bytes) > 10 * 1024 * 1024:
        st.error("Güvenlik nedeniyle 10MB limit mevcuttur.")
        return None
    try:
...
if st.session_state.page == "🏠 Atıf Motoru":
    ...
    st.markdown("""
        <div class="info-box">
            <p style="color: #e2e8f0; font-size: 16px; line-height: 1.6; margin-bottom: 0;">
                ...
                kullanarak saniyeler içinde hatasız referanslar oluşturur.
            </p>
        </div>
    """, unsafe_allow_html=True)

    style = st.selectbox("Atıf Formatı / Citation Style:", ["Vancouver", "APA 7th", "IEEE", "MLA"])
    t1, t2, t3 = st.tabs(["🔗 DOI", "🔍 Global Arama", "📄 PDF Analizi"])
    
    with t1:
        doi_in = st.text_input("DOI:", placeholder="10.1016/j.cell...")
        if st.button("Listeye Ekle", key="btn_doi"):
            res = get_cite(doi_in, True)
            if res: st.session_state.refs.append(res); st.rerun()

    with t2:
        q_in = st.text_input("Yayın Adı:", placeholder="Makale adını giriniz...")
        if st.button("Ara ve Ekle", key="btn_q"):
            res = get_cite(q_in, False)
            if res: st.session_state.refs.append(res); st.rerun()

    with t3:
        f = st.file_uploader("Dosya Seç", type="pdf")
        if f and st.button("Analiz Et"):
            res = process_pdf(f.read())
            if res: st.session_state.refs.append(res); st.rerun()
            else: st.warning("DOI bulunamadı.")
    
    # REFERANS LİSTESİ (Sadece atıflar)
    if len(st.session_state.refs) > 0:
        st.divider()
        txt_out = ""
        for i, r in enumerate(st.session_state.refs, 1):
            if style == "Vancouver": cite = f"{i}. {r['author']}. {r['title']}. {r['year']}."
            elif style == "APA 7th": cite = f"{r['author']} ({r['year']}). {r['title']}."
            elif style == "IEEE": cite = f"[{i}] {r['author']}, \"{r['title']}\", {r['year']}."
            else: cite = f"{r['author']}. \"{r['title']}.\" {r['year']}."
            
            # Tek tek kopyalanabilir bloklar
        ...
        with col1:
            st.download_button("📥 Tümünü TXT Olarak İndir", txt_out, use_container_width=True)
        with col2:
    ...
    with col_a:
        ...
        with st.expander("DOI Nedir?"): st.write("Makalelerin internetteki kalıcı kimliğidir.")
        with st.expander("Formatlar?"): st.write("Vancouver, APA 7, IEEE ve MLA desteklenir.")
    with col_b:
        st.markdown(f"### 🚀 {'Neden Citemate?' if st.session_state.lang == 'Türkçe' else 'Why Citemate?'}")
        st.write("* **Hız:** DOI ile anında sonuç.")
        st.write("* **Doğruluk:** Crossref veritabanı garantisi.")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("""<div class="service-card"><h3>🧬 Genetik Veri Analizi</h3><p>NCBI, GEO ve TCGA büyük veri setlerinin Python tabanlı işlenmesi.</p><span class="feature-tag">TCGA</span><span class="feature-tag">GEO</span><span class="feature-tag">NCBI</span></div>""", unsafe_allow_html=True)
        st.markdown("""<div class="service-card"><h3>🤖 Sağlıkta Makine Öğrenimi</h3><p>Klinik verilerle hastalık tahmin ve yapay zeka modelleri.</p><span class="feature-tag">Python</span><span class="feature-tag">ML / AI</span></div>""", unsafe_allow_html=True)
    with c2:
        st.markdown("""<div class="service-card"><h3>📊 Büyük Veri Analitiği</h3><p>Büyük ölçekli verilerin ileri istatistiksel raporlanması.</p><span class="feature-tag">Big Data</span><span class="feature-tag">Python</span></div>""", unsafe_allow_html=True)
        st.markdown("""<div class="service-card"><h3>🖋️ Referans Yazımı</h3><p>Yayınların Lifegenix uzmanlığıyla düzenlenmesi.</p><span class="feature-tag">Editorial Review</span></div>""", unsafe_allow_html=True)
